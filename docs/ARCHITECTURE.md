...
`engine.Run()` (`agent/engine.go`) 是统一的 Agent 循环，主 Agent 和 SubAgent 共享同一个实现。
...
AES-256-GCM，密钥从 `XBOT_ENCRYPTION_KEY` 环境变量加载（Base64 编码 32 字节）。**Graceful degradation**: 密钥未设置/无效时降级为明文存储（仅打 warn 日志）。
