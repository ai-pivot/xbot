...
- **panelRegistry**（`plugin-runtime/panelRegistry.ts`）：内置面板与插件面板的唯一注册表。`registerPanel/unregisterPanel/listPanels/subscribePanels`，通知模式仿 registry 的 view 订阅（listener 异常不中断广播）。def 契约在 `plugin-api/panels.ts`（`PanelDefinition`：id/title/labelKey?/icon/defaultSlot('left')/defaultMode('docked'|'floating')/defaultSize?/render(ctx)/badges?/source?('core'|pluginId)）。`labelKey` 为可选 i18n key（布局项 labelKey 同语义）：渲染处（PanelLayout/rails 的 PanelChrome title、chip/徽章 tooltip）`labelKey ? t(labelKey) : title` 优先；缺省回退 title（插件面板多传中文/英文 title，内置面板配 sidebar.* 等 key）。
