...
1. `refreshCachedModelName()` → 从 Session JSON 或 RPC 恢复 activeSubID/cachedModelName
2. `RefreshValuesCache(activeSubID)` — 同步 settings 缓存
3. `scheduleSessionLLMRestore()` — 异步触发后端 `SetSessionLLM` + `SwitchLLM`
...
### 4. scheduleSessionLLMRestore 的二次 SetDefault
...
### 5. remote mode 下 Session JSON 不是订阅的 source of truth
...
### 6. PerModelConfig 写入必须用 UpdatePerModelConfig
