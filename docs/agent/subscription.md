...
**失效**：`thinking_mode` 在 `SettingHandlerRegistry` 注册了 handler，`ApplyAgent` 调 `f.invalidateUserMemos(senderID)`（清 `sessionMemo`，ResolveLLM 缓存 thinking 的地方）+ `InvalidateSender`（清 `f.entries`）。`Agent.SetUserThinkingMode` 同样调 `invalidateUserMemos`。注意 `InvalidateSender` 只清 `f.entries`，不清 `sessionMemo`——必须用 `invalidateUserMemos`，否则切换后 `ResolveLLM` 命中旧 memo 返回旧 thinking。
...
**CLI `ApplySettings` 解耦**：`cmd/xbot-cli/main.go` 的 `ApplySettings` 把 `thinking_mode` 从 `llmFieldChanged` 移除，`updateActiveSubscription` 不再写 `sub.ThinkingMode`——切换 thinking 不会触发订阅 Update，只走 `ApplyRuntimeSettings`（→ handler → 失效 memo）。
