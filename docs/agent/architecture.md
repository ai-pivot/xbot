...
| 5 | ProjectContextMiddleware | `04_global_context` + `05_project_context` | Load AGENTS.md (global + project) — **bounded**: `maxProjectContextChars` (10k bytes) + `use Read tool` hint |
...
- **运行时切换**: `Ctrl+N` LLM 面板或 `/set-model <订阅名> <模型名>` 命令实时切换模型（跨订阅）
