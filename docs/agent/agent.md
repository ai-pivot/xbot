...
2. Post-compress check (`postCompressTokens > postCompressLimit`): `aggressiveTruncate` → re-estimate → still over (or nothing to truncate — few messages, giant system) → the fuse is **deleted** (2026-09-13): compression stays enabled — `aggressiveTruncate` still runs and a visible `compressWarning` tells the user to raise max_context / lower max_output, but nothing latches.
...
Anti-infinite-loop protection is now purely **rate limiting + safety nets**: the 5-iteration cooldown (`lastCompressIter`) throttles **auto** compression only, `aggressiveTruncate` bounds one round, and the error-driven paths (`handleInputTooLong`, `context_window_exceeded` with `maxCompressRetries=3`) still get full recovery attempts. Explicit `compact_context` requests run in **any** state (never vetoed by engine state); when they cannot run (too few messages / `mode=none`) they log a Warn + surface `compressWarning`.
