...
- **Policy: retry by DEFAULT (2026-09-11, user request "所有失败都要触发指数退避")** — `IsRetryableError` (`llm/retry.go`, exported so other request paths reuse the one implementation) returns true for everything except two semantic exceptions: `context.Canceled` (user `/cancel` — not a failure; retrying fights the user) and input-too-long (`IsInputTooLongError` — deterministic, and `agent.handleInputTooLong` has a dedicated compress-then-retry path, so spinning here only delays the real fix). It used to be a WHITELIST (429/5xx + net.Error + a hand-maintained keyword list); a whitelist can never enumerate every provider/gateway/SDK error shape, and every miss degraded to "fail once, no backoff". **4xx is retryable now** — a rejected request consumes no quota, and a proxy-rewritten transient failure is indistinguishable from a real client error in advance. Backoff: `Attempts` (default 5, `agent.llm_retry_attempts`) + `Delay=1s` + `MaxDelay=30s` + `CombineDelay(BackOffDelay, RandomDelay)` = exponential + jitter; 429 gets an extra 2s→32s backoff.
- **Every other request path must use the same predicate** — audit found `RefreshModelEntriesForUser`'s per-subscription `/models` fetch was the only HTTP call not wrapped by `RetryLLM` (`LoadModelsFromAPI` is a deliberate pass-through), so a transient blip silently left the model list unrefreshed. It now retries 3× with 1s/2s backoff, keeping its 8s per-attempt timeout.
...
- **Responses API (thinking mode) MUST pass reasoning back in `content` (`reasoning_text`) — `summary` alone is rejected with 400 (2026-09-12 fix).** Live error: `POST .../v1/responses: 400 Bad Request {"message":"The reasoning_text in the thinking mode must be passed back to the API.","type":"invalid_request_error"}` (tokendance gateway); it is NOT retryable, so every attempt fails and the whole turn dies. Root cause: `toResponsesParams` emitted only `Summary: [{Text: …}]` (→ `summary[].type=summary_text`) for assistant turns carrying `ReasoningContent`, while the gateway validates the `content[].type=reasoning_text` part. A reasoning input item has BOTH shapes — `summary` (`summary_text` parts) and `content` (`reasoning_text` parts); emit both (text verbatim). The non-streaming response parser now prefers `item.Content` and falls back to `item.Summary` (reading both would duplicate the reasoning text). Reasoning persistence was already intact (`session_messages.reasoning_content` column, written by `agent/engine_run.go`, restored by Replay), so this is purely a request-construction defect. Test: `llm/openai_responses_test.go` `TestToResponsesParams_AssistantReasoningCarriesReasoningText` (asserts the marshaled body contains `"type":"reasoning_text"` and the verbatim text; removing `Content` makes it red).
