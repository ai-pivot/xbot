...
- **`reconcileHistoryWithLiveRows` 绝不丢弃 live（persisted=false）行**：watermark（eventSeq）只对 **persisted** 消息生效。原因：**canceled turn** 的 DB 只有一条薄 `[interrupted]` 空 assistant 行（seq 高于 live），若 watermark 先于 `persisted===false` 判断，live assistant（可能含多个已完成迭代）在用户发下一条消息时被丢弃——内容凭空消失。live 行仍与 history 按 `turnID:role`（`turnID>0`）或 `content:role`（`turnID=0` legacy）去重，确保不重复渲染。
- **loadMore 滚动锚定依赖 id 稳定，锚点失效时用 scrollHeight 差补偿**：锚定 id 取第一个可见行，但 real-time echo→DB row 的 dedup 合并（`dedupMessages`，progressStore.ts，dbID 优先）会把 `echo-xxx` 替换成 `db-xxx` → `findIndex` 返回 -1 → 视口跳到顶部。修复（MessageList.tsx）：prepend 前记录 `prevScrollHeightRef`，锚点 id 消失（newIdx<0）时按 `scrollTop + (prevScrollHeight - newScrollHeight)` 补偿保持可视内容位置；锚点存在则 `scrollToIndex(anchorId, {align:'start'})`。
