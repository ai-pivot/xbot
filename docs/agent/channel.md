...
- **⚠️ 行尺寸的唯一真相 = 「行元素自己的当前几何」；RO 快照只允许用来【触发】测量（2026-09-23，`entry.borderBoxSize` 作尺寸的整类根因根治）**：虚拟行是 `position: absolute; transform: translateY(start)`，`start` = 前面所有行尺寸的累加 ⇒ **只要某行的"记账尺寸"小于它的真实高度，下一行就画到它身上**（用户报告：正文互相穿插 / 两段文字压在同一 y）。而 RO 的 `entry.borderBoxSize` 是**观察时刻的快照、不是当前几何**：乱序/滞后投递（后台标签页/面板隐藏后恢复、RO 投递被推迟）时它比真实尺寸**小**，旧实现把它当尺寸写回虚拟器（`resizeItem`）⇒ 行被缩矮；随后 DOM 不再变化 ⇒ 没有下一次回调 ⇒ **错值永久固化**（画面静止也一样错，与 2026-09-21「追加行必须权威重测」是同一根因的第二次暴露 —— 当时"测量源改读当前几何"只落在 `noDegenerateMeasureElement` 的**无 entry 兜底分支**，主路径仍信快照）。**现在的架构（`MessageList.tsx` 的 `flushMeasure` + `rowHeightMemory.ts`）**：① `createHeightAwareMeasureElement` 有 entry 时**绝不返回快照尺寸** —— 只 `onResize()` 标脏并返回虚拟器当前记账尺寸（不变 ⇒ `resizeItem` 早退）；② 唯一写入点 `flushMeasure`：**按 DOM 顺序枚举已挂载行**（`data-index` 为准，不按虚拟键查表 ⇒ 键冲突/表未命中都不能漏行）→ **一次读完所有行的真几何**（读之间无写 ⇒ 整批一次布局）→ 写尺寸（`resizeItem`）→ ③ **按真实高度逐行累加直接写 `transform`**（以窗口首行 `start` 为锚）⇒ **"两行压在同一 y"在结构上不可能**，哪怕虚拟器记账因任何原因滞后一帧；④ 实测高度写进 `heightMemory`，仅作**未挂载行**的 `estimateSize` 初值（永远会被 flush 校正）。三个触发点共用同一 flush：每次 commit（layout effect 内同步跑，paint 前）、RO 回调（microtask 合帧）、live 行长高的低频兜底（`frameScheduler`）。诊断标记：`data-row-size`（DEV，虚拟器记账尺寸）/ `data-measure-pass` / `data-virt-total`。守护：`src/components/agent/rowHeightMemory.test.ts`（过期快照绝不允许缩行 —— **已验证 master 上必红**）、`src/components/agent/virtualizer_stale_row.test.ts`、`rowMeasureInvariant.test.ts`、`e2e/row-measure-invariant.spec.ts`（真实 Chromium：历史加载 + 流式追加 + 滚动 + **不经过 React 的异步长高**，全程断言「记账尺寸 == 实际高度」且「任意两行不重叠」）、`e2e/standalone-command-layout.spec.ts`。
