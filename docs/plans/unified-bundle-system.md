...
- [x] `xbot.json` schema 定义完成，`AppManifest` 结构体实现
- [x] `AppPackager.Pack` / `Unpack` / `Validate` 实现
- [x] DB v45 迁移完成（CHECK 约束 + version 列）
- [x] `RegistryManager.PackApp` / `InstallAppFromFile` 实现（仅 skill + agent）
- [x] `/app export` 和 `/app install <file>` 命令实现
- [x] 单元测试：打包 → 解包 → 安装全流程
- [x] `go build ./...` 通过
- [x] `go test ./agent/... ./storage/...` 通过

### Phase 2：统一市场 ✅
- [x] `RegistryManager.PublishApp` / `InstallApp` 实现
- [x] `/app browse` / `/app publish` / `/app install <id>` 支持 app 类型
- [x] Web API `/api/market?type=app` 可用
- [x] 飞书市场卡片新增 app 标签页
- [x] `/app uninstall` 和 `/app list` 子命令实现
- [x] 旧命令 `/browse` / `/install` / `/publish` 等保留兼容
- [x] `go build ./...` 通过
- [x] `go test ./...` 通过

### Phase 3：Plugin 纳入 ✅
- [x] `RegistryManager` 持有 `xbotHome` 用于定位 plugins 目录
- [x] `stagePlugin` — 打包时读取 plugin.json，复制整个插件目录到 zip
- [x] `installAppPlugin` — 从 zip 解包后复制到 `~/.xbot/plugins/<id>/`
- [x] `uninstallPlugin` — 从 plugins 目录删除插件
- [x] `ListInstalledPlugins` — 列出已安装插件
- [x] `findPluginDir` — 按 ID/名称/目录名查找插件
- [x] `/app export --plugin <name>` 命令支持
- [x] `/app list` 显示已安装 plugins
- [x] `Uninstall("plugin", ...)` 支持
- [x] `ListMy` 扫描本地 plugins 目录
- [x] 单元测试：包含 plugin 的 pack→unpack→validate 全流程
- [x] `go build ./...` 通过
- [x] `go test ./...` 通过
- [ ] **未纳入**（执行层范畴）：安装前 permissions 确认 UI、Plugin 依赖检查、PluginManager 运行时激活 — 这些属于插件执行层，包管理器仅负责文件分发

### Phase 4：增强功能（可选）
### Phase 4：URL 安装 + 包签名 ✅
- [x] `/app install <url>` — 从 HTTP/HTTPS URL 下载并安装 .xbot.zip
- [x] 下载限制 100MB，超限拒绝
- [x] `AppManifest.Checksum` 字段 — SHA256 签名（Pack 时计算，Unpack 时验证）
- [x] 签名验证：篡改包内容时 Unpack 返回 checksum mismatch 错误
- [x] 移除内部市场功能（browse/publish/unpublish/my/install-by-id）
- [x] 移除旧命令 `/browse` `/install` `/publish` `/unpublish` `/uninstall` `/my`
- [x] 移除 Web API 市场端点（/api/market/*），保留 /api/app/{pack,install,uninstall}
- [x] 移除 RegistryCallbacks 中的市场回调字段
- [x] 安装时保存 app manifest 到 `~/.xbot/installed_apps/<name>.json`（用于卸载追踪）
- [x] `go build ./...` 通过
- [x] `go test ./...` 通过
...
1. ~~命名~~：已确定使用 `/app`。
2. ~~Plugin 纳入时机~~：Phase 3 已完成。
3. **旧命令**：已移除。`/browse` `/install` `/publish` `/unpublish` `/uninstall` `/my` 全部删除，统一由 `/app` 子命令替代。
4. ~~文件扩展名~~：`.xbot.zip`。
5. **Feishu 市场卡片**：`AppMarketPage` 字段目前为死代码（编译通过），可后续清理。
6. **更新检测**：Phase 4 未实现版本对比，如有需要可后续添加。
