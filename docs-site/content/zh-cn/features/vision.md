...
- 工具会读取图片、存储、并注入到下一轮的视觉上下文
- **注入的那条消息是「给模型看的内部载体」，不会出现在对话历史里** —— 它复用你这条消息的 turn，只负责把图片送给模型（OpenAI 的 tool 消息不能带图）。你在界面上看到的始终只有**你自己发的那条**消息（含你上传的图片原件）
...
> 历史里存着这次注入，因此后续轮次模型仍能看到它当时分析过的图（例如"再改一版"时对比上一版），但界面上不重复显示，也不会顶掉你的消息。
