...
### `AGENTS.md` 必须控制在注入预算内

`AGENTS.md` 会在每次请求时注入 system prompt，因此有 **10 万字符的硬预算**
（`agent/middleware_builtin.go` 的 `maxProjectContextChars`）。超出部分会被截断，
并告知模型缺失内容是「不可见」的，同时要求它把文件缩短到预算内。

对贡献者的两条要求：

- **根目录 `AGENTS.md` 保持精简**（快速参考 + 知识文件索引 + 规则一行索引），
  细节放进 `docs/agent/*.md` —— 那些文件是按需 Read 的，没有预算限制。
- **注意单位**：预算按**字符（rune）**计，不是字节 —— 一个中文字符占 3 个 UTF-8 字节，
  所以 690 KB 的文件其实是约 50 万字符。tokenizer 大致是「中文 1 token/字符、
  ASCII 0.25 token/字符」，即 10 万字符最坏约等于 10 万 token（约 200k 上下文窗口的一半）。

为什么必须强制：压缩只会重写对话**消息**，永远不会改 system prompt。
`AGENTS.md` 过大会让「不可压缩部分」自己就超过模型上下文，
于是每次压缩都无效，会话会无限循环压缩、模型持续复读。
