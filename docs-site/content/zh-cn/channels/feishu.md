...
Agent 工作期间，确认语、工具进度与最终回复会流式写入同一张飞书 **CardKit 流式卡片**
（Card JSON 2.0 + `config.streaming_mode: true`）。卡片形态对齐飞书官方 Agent 卡片：

- 彩色标题栏 + 阶段副标题；
- **可折叠时间线**「🛠️ 执行过程 · N 步」—— 每个工具一步，执行中自动展开、全部结束后折叠；
- 回复正文，以打字机效果流式输出；
- 小灰字状态页脚。
...
- 应用需具备 **创建与更新卡片** 权限（`cardkit:card:write`）。缺少该权限时，渠道只打印
  一次警告并回退到静态卡片。
- 进度推送有节流（正文约 4 次/秒、时间线约 1 次/秒）；最终回复一定会写入最终文本并关闭
  流式模式。
