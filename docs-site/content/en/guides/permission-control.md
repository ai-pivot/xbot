...
Permission requests flow through the hooks system. The `PermissionRequest` and `PermissionDenied` lifecycle events allow custom hook handlers to:

- Intercept permission requests before they reach the user
- Auto-approve or auto-deny specific patterns
- Log all permission requests for audit trails
- Integrate with external approval systems (e.g., PagerDuty, Slack)

Configure hooks in `~/.xbot/hooks.json`:
