...
- Name: right-sidebar.spec.ts >> Right sidebar (Spec 6) >> session config shows info and switches model
- Location: e2e/right-sidebar.spec.ts:97:3
...
Call log:
  - waiting for locator('.flex.h-full.w-12.shrink-0.flex-col').last().locator('button[aria-pressed]').nth(3)
