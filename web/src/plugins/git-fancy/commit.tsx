...
export function GitCommitView({ hash }: GitCommitViewProps) {
  ...
  useEffect(() => {
    ...
    if (!hash || !getRpc()) {
      setLoading(false)
      setError(!hash ? t('missingHash', '缺少 hash 参数') : t('pluginNotInitialized', 'Git 插件未初始化'))
      return
    }
  ...
  if (loading) {
    return <div className="flex h-full items-center justify-center text-xs text-text-muted">{t('loadingCommit', '加载 commit 详情…')}</div>
  }
  ...
  const metaHeader = (
    <div className="border-b border-border px-3 py-2">
      <div className="flex flex-wrap items-center gap-2">
        ...
        <span className="text-xs text-text-primary">{t('fileChanges', '{{count}} 个文件变更', { count: detail.files.length })}</span>
  ...
  const fileRows = detail.files.map((f) => {
    ...
    return (
      <div
        ...
        title={t('openCommitFileDiff', '在编辑区查看此 commit 内该文件的 diff')}
      >
        <span
