...
describe('TDSM reduce — 历史 P0 回归', () => {
  ...
  it('REPRO: todo_write 后 history_replaced 不覆盖实时 todos（快照旧值覆盖根因）', () => {
    ...
    const s0 = run([
      { type: 'iteration', turnID: T1, iter: iterNum(1), seq: 10 as never, content: undefined, reasoning: undefined, activeTools: [], completedTools: [], iterationsDelta: [], todos: [{ text: '新任务', status: "pending" }, { text: '任务2', status: "pending" }], subAgents: undefined, tokenUsage: undefined, streamStats: undefined },
    ])
