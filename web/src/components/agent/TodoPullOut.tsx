...
export function TodoPullOut({ todoState, hasGoal, onSetGoal }: TodoPullOutProps) {
  ...
  return (
    <div className="mx-2 mb-1.5 overflow-hidden rounded-md border border-border bg-sidebar-bg text-sm">
      <div className="flex h-8 w-full items-center gap-2 px-2.5 text-left">
        <button
          ...
          className="flex min-w-0 flex-1 items-center gap-2 text-left transition-colors hover:bg-surface-bg -mx-2.5 px-2.5 h-full"
        >
          <ChevronRight
            className={cn('size-3.5 shrink-0 text-text-muted transition-transform', expanded && 'rotate-90')}
          />
          <div className="h-1.5 w-12 shrink-0 overflow-hidden rounded-full bg-surface-bg">
            <div
