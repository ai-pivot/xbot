...
test.describe('卡片 Ctrl 拖动（真实浏览器）', () => {
  ...
  test('显式把手拖动（无 Ctrl）+ overlay 持续性（用户报告：闪一下就消失）', async ({ browser }) => {
    const page = await browser.newPage({ viewport: { width: 1400, height: 900 } })
    const pageErrors: string[] = []
    page.on('pageerror', (e) => pageErrors.push(String(e)))
    await setupMock(page)
    ...
    // pointer 事件序列探针（定位手势中断：up/cancel 时机）
    await page.evaluate(() => {
      const w = window as unknown as { __ptrLog: unknown[] }
      w.__ptrLog = []
      const log = (type: string) => (e: PointerEvent) => {
        w.__ptrLog.push({ type, id: e.pointerId, x: Math.round(e.clientX), y: Math.round(e.clientY), ts: Date.now() })
      }
      window.addEventListener('pointerdown', log('down'), true)
      window.addEventListener('pointermove', log('move'), true)
      window.addEventListener('pointerup', log('up'), true)
      window.addEventListener('pointercancel', log('cancel'), true)
    })

    // 非 Tab 卡（sessions）的显式把手条（751a1558 LayoutEngine 注入）
    const handleBar = page.locator('.card-drag-handle-bar').first()
    await expect(handleBar).toBeVisible({ timeout: 10_000 })
    const hb = await handleBar.boundingBox()
    ...
    await page.mouse.down()
    await page.mouse.move(hx, hy + 40, { steps: 8 })
    await page.waitForTimeout(150)

    // overlay 持续性采样（每 120ms x 5——用户报告"闪一下就不再出现"）
    const samples: number[] = []
    const cursorSamples: string[] = []
    for (let i = 0; i < 5; i++) {
      samples.push(await page.locator('body > div[style*="z-index: 9999"]').count())
      cursorSamples.push(await page.evaluate(() => document.body.style.cursor))
      await page.waitForTimeout(120)
    }
    const ptrLog = await page.evaluate(() => (window as unknown as { __ptrLog?: unknown[] }).__ptrLog)
    console.log('[e2e-diag] handle-drag overlay samples:', JSON.stringify(samples), 'cursor:', JSON.stringify(cursorSamples))
    console.log('[e2e-diag] pointer log (last 12):', JSON.stringify((ptrLog ?? []).slice(-12)))

    await page.mouse.up()
    await page.waitForTimeout(300)

    // 把手拖动应激活（overlay 或 grabbing 二者其一，且持续不闪烁）
    const everActivated = samples.some((c) => c > 0) || cursorSamples.some((c) => c === 'grabbing')
    console.log('[e2e-diag] everActivated:', everActivated, 'pageErrors:', pageErrors)
    // 诊断核心：若激活后 overlay 中途消失（闪一下），samples 序列将呈 [1,0,0,0,0]
    expect(pageErrors, `page errors: ${pageErrors.join('; ')}`).toEqual([])
  })
})
