...
func TestFullHistoryCompressionSourcesExcludeHiddenPruneControls(t *testing.T) {
	...
	pruneID, err := svc.AppendContextSnapshot(tenantID, HistoryRecordPrune, []llm.ChatMessage{
		...
		{ID: answerID, Role: "assistant", Content: "old answer"},
	})
	if err != nil {
		t.Fatal(err)
	}
	...
	compressID, err := svc.AppendContextSnapshot(tenantID, HistoryRecordCompress, []llm.ChatMessage{
		{ID: laterID, Role: "user", Content: "later"},
	})
	if err != nil {
		t.Fatal(err)
	}
...
func TestReplayCompressionContextEditAndAskUser(t *testing.T) {
	...
	if len(replay.Messages) != 2 || replay.Messages[0].Content != "summary" || replay.Messages[1].ID != answerID {
		t.Fatalf("compressed replay=%+v", replay.Messages)
	}

	summary := replay.Messages[0]
	summary.Content = "edited summary"
	if _, err := svc.AppendControl(tenantID, HistoryRecordContextEdit, summary.ID,
		MessageMutations{Mutations: []MessageMutation{{TargetHistoryID: summary.ID, Message: summary}}}); err != nil {
		t.Fatal(err)
	}
...
func TestReplayTargetsSyntheticSnapshotMessageByOccurrence(t *testing.T) {
	...
	if _, err := svc.AppendControl(tenantID, HistoryRecordContextEdit, target.ID, MessageMutations{Mutations: []MessageMutation{
		{TargetHistoryID: target.ID, TargetOccurrence: 0, Message: target},
	}}); err != nil {
		t.Fatal(err)
	}
	replay, err = svc.Replay(tenantID)
...
func TestAppendContextSnapshotRejectsUnknownOrInactiveHistoryID(t *testing.T) {
	...
	if _, err := svc.AppendContextSnapshot(tenantID, HistoryRecordPrune, []llm.ChatMessage{
		{ID: second + 999, Role: "user", Content: "unknown"},
	}); err == nil {
	...
	if _, err := svc.AppendContextSnapshot(tenantID, HistoryRecordPrune, []llm.ChatMessage{
		{ID: first, Role: "user", Content: "stale"},
	}); err == nil {
...
func TestConcurrentSnapshotAndMutationsRemainReplayable(t *testing.T) {
	...
	for i := 0; i < iterations; i++ {
		...
		go func() {
			<-start
			_, err := writerB.AppendContextSnapshot(tenantID, HistoryRecordPrune, []llm.ChatMessage{{ID: keepID, Role: "user", Content: "keep"}})
			errs <- err
		}()
		close(start)
