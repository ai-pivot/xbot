	...
	err := s.withImmediateHistoryWrite(func(store historyQueryExecer) error {
		...
		for _, msg := range messages {
			if msg.Role != "system" && !msg.DisplayOnly {
				if msg.ID != 0 {
					occurrence := occurrences[msg.ID]
					if activeMessageIndexOccurrence(replay.Messages, msg.ID, occurrence) < 0 {
						return fmt.Errorf("%s snapshot history_id %d occurrence %d is not active", recordType, msg.ID, occurrence)
					}
					occurrences[msg.ID] = occurrence + 1
				}
				active = append(active, msg)
				historyIDs = append(historyIDs, msg.ID)
			}
		}
...
func validateAndAppendAskQuestionWith(store historyQueryExecer, tenantID int64, metadata map[string]string) (int64, error) {
	...
	for i := len(replay.Messages) - 1; i >= 0; i-- {
		if replay.Messages[i].Role == "tool" && replay.Messages[i].ToolName == "AskUser" {
			toolHistoryID = replay.Messages[i].ID
			break
		}
	}
	if toolHistoryID == 0 {
	...
	for recordIndex, record := range records {
		...
		case HistoryRecordCompress, HistoryRecordPrune:
			...
			for i := range result.Messages {
				result.Messages[i].ID = snapshot.HistoryIDs[i]
				if result.Messages[i].ID == 0 {
					result.Messages[i].ID = record.HistoryID
				} else if !hasCheckpoint || recordIndex != 0 {
					if _, ok := known[result.Messages[i].ID]; !ok {
		...
		case HistoryRecordContextEdit:
			...
			for _, mutation := range mutations.Mutations {
				...
				mutation.Message.ID = mutation.TargetHistoryID
