...
func TestApplyCompressAssignsSyntheticHistoryIDForSameRunEdit(t *testing.T) {
	...
	if compressed.NewMessages[1].ID == 0 {
	...
	if replayed[0].ID != compressed.NewMessages[1].ID || replayed[0].Content == "synthetic summary that can be truncated" {
...
func TestIncrementalPersistBatchFailureRetriesWithoutDuplicates(t *testing.T) {
	...
	for i, msg := range messages {
		if msg.ID == 0 || records[i].HistoryID != msg.ID {
...
func TestPersistenceBridgeAppendFailureDoesNotAdvanceWatermark(t *testing.T) {
	...
	if messages[0].ID != 0 {
		t.Fatalf("history ID assigned on failure: %d", messages[0].ID)
	}
}
...
func TestAggressiveTruncateAppendFailureKeepsEditorOnOldContext(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		messages = append(messages, llm.ChatMessage{ID: int64(i + 1), Role: "user", Content: fmt.Sprintf("m%d", i)})
	}
	editor := NewContextEditor(NewContextEditStore(10))
	editor.SetMessages(messages)
...
func TestContextEditorRollsBackWhenHistoryAppendFails(t *testing.T) {
	messages := []llm.ChatMessage{
		{ID: 1, Role: "user", Content: "0123456789"},
		{ID: 2, Role: "assistant", Content: "old"},
		{ID: 3, Role: "user", Content: "protected1"},
		{ID: 4, Role: "assistant", Content: "protected2"},
		{ID: 5, Role: "user", Content: "protected3"},
	}
	editor := NewContextEditor(NewContextEditStore(10))
	editor.SetMessages(messages)
