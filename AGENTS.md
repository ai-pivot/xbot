...
- ⛔ `docs/agent/gotchas-web-frontend.md` — 压缩标记（`[Compacted context]`）绝不参与 turn 推导，且**渲染在 Iter 之间**（Cursor 式，2026-09-26 P0：用户发的「继续」被**更早**的压缩标记顶掉 —— `deriveTurnIDs` 把 turn_id=0 的标记绑到了它**之后**的 turn ⇒ 前端「每 turn 取第一条 user」让标记抢占用户槽位；重新设计：压缩点归属它所在的 turn（`HistoryMessage.Compactions{AfterIteration}`，由 `IterationRecord.CreatedAt` 定位），前端在迭代之间渲染 `CompactionDivider`；老数据回落 standalone 行）。
