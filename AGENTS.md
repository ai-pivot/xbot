...
- ⛔ `docs/agent/gotchas-web-frontend.md` — 压缩标记（`[Compacted context]`）绝不参与 turn 推导（2026-09-26 P0：用户发的「继续」被**更早**的压缩标记顶掉 —— `deriveTurnIDs` 把 turn_id=0 的标记绑到了它**之后**的 turn ⇒ 前端「每 turn 取第一条 user」让标记抢占用户槽位；修复 = 与 `CommandRow` 同款 standalone+anchor）。
