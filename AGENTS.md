...
- **`applyScrollbar` must truncate lines to `contentWidth-1` before appending scrollbar.** BubbleTea textinput `placeholderView()` outputs `SetWidth(N)+1` chars. Without truncation, the line reaches exactly `contentWidth`, pushing the scrollbar `▐` to the next line. Fix: `visW >= contentWidth` → `ansi.Truncate(line, contentWidth-1, "")`. The `>=` (not just `>`) is critical — lines at exactly `contentWidth` must also be truncated to leave room for at least 1 space padding.
...
- **Remote CLI `ChatRenameFn` must use upsert (INSERT ON CONFLICT), not UPDATE.** CLI sessions may not have a `user_chats` row (created locally, never INSERT'd). Plain `UPDATE` silently affects 0 rows. The upsert creates the row if missing.
...
- **`SyncLayoutSettings` must ONLY use `asyncCh`, never call model methods directly.** `refreshRemoteValuesCache` runs in a background goroutine. Direct calls to `c.model.applyLayoutConfig()` or `c.model.relayoutViewport()` race with the BubbleTea event loop, causing glamour table render panics (`index out of range` in lipgloss table resize). ALWAYS route through `asyncCh` → `cliSettingsSavedMsg` → `handleSettingsSavedMsg`. Note: `cliSettingsSavedMsg` carries `syncOnly: true` from `SyncLayoutSettings` to prevent `handleSettingsSavedMsg` from resetting context bar caches (`cachedMaxContextTokens` etc.) — doing so every 5s in remote mode caused the context bar to flash to solid line repeatedly.
