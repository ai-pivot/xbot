...
- **`ResetAfterCompress` must zero ALL tracker fields** (promptTokens, completionTokens, msgCountAtCall, hadLLMCall). Any non-zero estimate causes `maybeCompress` to re-trigger immediately, creating infinite compression loops. The tracker returns "no_data" until the next real LLM API call.
