...
- **`SanitizeMessages()` is the final validation gate before sending to LLM.** Three passes: (1) strip assistant messages with empty content AND no tool_calls — rejected by OpenAI/Anthropic; (2) strip tool_calls with invalid/malformed JSON arguments — caused by cancelled stream with partially-generated tool arguments, DeepSeek rejects with 400; (3) strip trailing unpaired tool_calls from cancelled runs. Called in `buildPrompt` and `engine.go`.
