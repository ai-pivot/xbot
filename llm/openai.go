...
func (o *OpenAILLM) processStream(ctx context.Context, stream *ssestream.Stream[openai.ChatCompletionChunk], eventChan chan<- StreamEvent, startTime time.Time, messages []ChatMessage, model string, tools []ToolDefinition, thinkingMode string) {
	...
	if lastFinishReason == "" && hasToolCalls {
		// Stream ended cleanly but no finish_reason — if any accumulated tool call
		// arguments are truncated (invalid JSON), the stream was cut mid-tool-call
		// (proxy/network). The old behavior passed the truncated JSON downstream as a
		// "complete" tool call → "parse args: unexpected end of JSON input" → LLM
		// retried blindly (token burn). EventError triggers RetryLLM which regenerates
		// the whole request.
		//
		// NOTE (rollback 2026-08-31): this json.Valid gate was TOO AGGRESSIVE — it
		// rejected legitimately formatted tool calls (e.g. task_status with array
		// params) and caused infinite retry loops. Reverted to simple finish_reason
		// inference (the original BUG 1 fix behavior before the gate was added).
		// The gate in CollectStreamWithCallbackFrom (stream.go) was also reverted —
		// upstream chunk corruption must be fixed at the gateway, not by rejecting
		// every malformed JSON in the consumer (which can't distinguish corruption
		// from provider-specific formatting).
