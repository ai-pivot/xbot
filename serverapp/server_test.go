...
// TestGetContextUsageRPC_ExactSnapshotFallback verifies get_context_usage:
// exact message snapshot → token-state fallback → global config fallback.
// (The multi-user authorization section was removed with the multi-user
// system — v63: every RPC caller IS the single operator.)
func TestGetContextUsageRPC_ExactSnapshotFallback(t *testing.T) {
	dir := t.TempDir()
	t.Setenv("XBOT_HOME", dir)
