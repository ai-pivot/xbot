...
// The PerModelConfig write + Invalidate on the server already propagates the
// change correctly.
func TestSaveSettings_MaxContextNotPassedToApplySettings(t *testing.T) {
	...
	if appliedKeys["max_context_tokens"] {
		t.Error("max_context_tokens reached ApplySettings — it should only go through PerModelConfig, " +
...
// max_context via PerModelConfig does NOT zero out an existing MaxOutputTokens
// for the same model.
...
// max_context for model-a does NOT touch model-b's PerModelConfig.
...
func (s *mockLLMSubscriber) SwitchSubscription(senderID string, sub *Subscription, chatID string) error {
	return nil
}
