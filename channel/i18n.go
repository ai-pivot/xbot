...
type UILocale struct {
	...
	ModelUsage          string // "用法: /set-model <模型名>\n使用 /models 查看可用模型"
...
func LocaleZH() *UILocale {
	return &UILocale{
		...
		HelpCmds: []HelpCmdEntry{
			...
			{Cmd: "/clear", Desc: "清空当前 TUI 显示"},
			{Cmd: "/copy", Desc: "复制消息到剪贴板"},
			{Cmd: "/compress", Desc: "压缩上下文"},
			{Cmd: "/set-llm", Desc: "创建/更新个人 LLM 订阅"},
			{Cmd: "/unset-llm", Desc: "删除指定个人订阅"},
			{Cmd: "/llm", Desc: "查看当前订阅与模型"},
			{Cmd: "/models", Desc: "列出可选模型"},
			{Cmd: "/set-model", Desc: "切换模型"},
			{Cmd: "/usage", Desc: "查看 token 用量"},
			{Cmd: "/new", Desc: "归档记忆并重置当前对话"},
			{Cmd: "/rename", Desc: "重命名当前会话"},
			{Cmd: "/rewind", Desc: "回退对话"},
			{Cmd: "/settings", Desc: "打开设置面板"},
			{Cmd: "/app", Desc: "应用管理（打包、安装、卸载）"},
			{Cmd: "/update", Desc: "检查更新"},
			{Cmd: "/help", Desc: "显示此帮助"},
			{Cmd: "/channel", Desc: "打开渠道配置面板"},
			{Cmd: "/chat", Desc: "创建、切换或列出会话"},
			{Cmd: "/debug", Desc: "查看运行时诊断信息"},
			{Cmd: "/exit", Desc: "退出 xbot-cli"},
			{Cmd: "/link-account", Desc: "生成或消费账号关联码"},
			{Cmd: "/list-sessions", Desc: "列出后端全部会话（仅管理员）"},
			{Cmd: "/palette", Desc: "打开命令面板"},
			{Cmd: "/plugin", Desc: "管理插件"},
			{Cmd: "/quit", Desc: "退出 xbot-cli"},
			{Cmd: "/search", Desc: "搜索消息历史"},
			{Cmd: "/sessions", Desc: "打开会话面板"},
			{Cmd: "/setup", Desc: "打开初始设置向导"},
			{Cmd: "/ss", Desc: "打开会话面板"},
			{Cmd: "/su", Desc: "切换身份或会话"},
			{Cmd: "/tasks", Desc: "打开后台任务与 Agent 面板"},
			{Cmd: "/unlink-account", Desc: "查看当前账号关联身份"},
			{Cmd: "/user", Desc: "管理 Web 用户"},
			{Cmd: "/version", Desc: "显示版本信息"},
			{Cmd: "/prompt", Desc: "预览完整提示词"},
			{Cmd: "/llms", Desc: "列出个人 LLM 订阅"},
			{Cmd: "/continue", Desc: "继续被中断的上一轮对话"},
			{Cmd: "/context mode", Desc: "查看或切换压缩模式"},
			{Cmd: "/context", Desc: "查看上下文统计"},
			{Cmd: "/plugin reload-all", Desc: "重新加载所有插件"},
		...
		HelpKeys: []HelpKeyEntry{
			...
			{Key: "Ctrl+N", Desc: "LLM 面板（切换模型 + 管理订阅）"},
...
func localeEN() *UILocale {
	return &UILocale{
		...
		ModelUsage:          "Usage: /set-model <model name>\nUse /models to list available models",
		...
		HelpCmds: []HelpCmdEntry{
			...
			{Cmd: "/clear", Desc: "Clear the current TUI display"},
			...
			{Cmd: "/unset-llm", Desc: "Delete a named personal subscription"},
			{Cmd: "/llm", Desc: "Show current subscription and model"},
			{Cmd: "/models", Desc: "List available models"},
			{Cmd: "/set-model", Desc: "Switch model"},
			{Cmd: "/usage", Desc: "View token usage"},
			{Cmd: "/new", Desc: "Archive memory and reset this conversation"},
			{Cmd: "/rename", Desc: "Rename current session"},
			{Cmd: "/rewind", Desc: "Rewind conversation"},
			{Cmd: "/settings", Desc: "Open settings panel"},
			{Cmd: "/app", Desc: "App management (pack, install, uninstall)"},
			{Cmd: "/update", Desc: "Check for updates"},
			{Cmd: "/help", Desc: "Show this help"},
			{Cmd: "/channel", Desc: "Open channel configuration"},
			{Cmd: "/chat", Desc: "Create, switch, or list sessions"},
			{Cmd: "/debug", Desc: "Show runtime diagnostics"},
			{Cmd: "/exit", Desc: "Exit xbot-cli"},
			{Cmd: "/link-account", Desc: "Create or consume an account link code"},
			{Cmd: "/list-sessions", Desc: "List all backend sessions (admin only)"},
			{Cmd: "/palette", Desc: "Open command palette"},
			{Cmd: "/plugin", Desc: "Manage plugins"},
			{Cmd: "/quit", Desc: "Exit xbot-cli"},
			{Cmd: "/search", Desc: "Search message history"},
			{Cmd: "/sessions", Desc: "Open the Sessions panel"},
			{Cmd: "/setup", Desc: "Open the setup wizard"},
			{Cmd: "/ss", Desc: "Open the Sessions panel"},
			{Cmd: "/su", Desc: "Switch identity or session"},
			{Cmd: "/tasks", Desc: "Open background tasks and Agents"},
			{Cmd: "/unlink-account", Desc: "Show linked account identities"},
			{Cmd: "/user", Desc: "Manage Web users"},
			{Cmd: "/version", Desc: "Show version information"},
			{Cmd: "/prompt", Desc: "Preview the complete prompt"},
			{Cmd: "/llms", Desc: "List personal LLM subscriptions"},
			{Cmd: "/continue", Desc: "Continue the interrupted previous turn"},
			{Cmd: "/context mode", Desc: "Show or change compression mode"},
			{Cmd: "/context", Desc: "Show context statistics"},
			{Cmd: "/plugin reload-all", Desc: "Reload all plugins"},
		...
		HelpKeys: []HelpKeyEntry{
			...
			{Key: "Ctrl+N", Desc: "LLM panel (switch model + manage subscriptions)"},
...
func localeJA() *UILocale {
	return &UILocale{
		...
		HelpCmds: []HelpCmdEntry{
			...
			{Cmd: "/clear", Desc: "現在の TUI 表示をクリア"},
			{Cmd: "/copy", Desc: "メッセージをクリップボードにコピー"},
			{Cmd: "/compress", Desc: "コンテキストを圧縮"},
			{Cmd: "/set-model", Desc: "モデル切替"},
			{Cmd: "/models", Desc: "利用可能モデル一覧"},
			{Cmd: "/set-llm", Desc: "個人 LLM サブスクリプション作成/更新"},
			{Cmd: "/unset-llm", Desc: "指定した個人サブスクリプションを削除"},
			{Cmd: "/llm", Desc: "現在のサブスクリプションとモデルを表示"},
			{Cmd: "/usage", Desc: "トークン使用量を表示"},
			{Cmd: "/new", Desc: "メモリを保存して現在の会話をリセット"},
			{Cmd: "/rename", Desc: "現在のセッション名を変更"},
			{Cmd: "/rewind", Desc: "会話を巻き戻す"},
			{Cmd: "/settings", Desc: "設定パネルを開く"},
			{Cmd: "/app", Desc: "アプリ管理（パック、インストール、アンインストール）"},
			{Cmd: "/update", Desc: "アップデート確認"},
			{Cmd: "/help", Desc: "ヘルプ表示"},
			{Cmd: "/channel", Desc: "チャンネル設定を開く"},
			{Cmd: "/chat", Desc: "セッションの作成・切替・一覧表示"},
			{Cmd: "/debug", Desc: "ランタイム診断を表示"},
			{Cmd: "/exit", Desc: "xbot-cli を終了"},
			{Cmd: "/link-account", Desc: "アカウント連携コードを作成または使用"},
			{Cmd: "/list-sessions", Desc: "全バックエンドセッション一覧（管理者のみ）"},
			{Cmd: "/palette", Desc: "コマンドパレットを開く"},
			{Cmd: "/plugin", Desc: "プラグイン管理"},
			{Cmd: "/quit", Desc: "xbot-cli を終了"},
			{Cmd: "/search", Desc: "メッセージ履歴を検索"},
			{Cmd: "/sessions", Desc: "セッションパネルを開く"},
			{Cmd: "/setup", Desc: "セットアップウィザードを開く"},
			{Cmd: "/ss", Desc: "セッションパネルを開く"},
			{Cmd: "/su", Desc: "ID またはセッションを切替"},
			{Cmd: "/tasks", Desc: "バックグラウンドタスクと Agent を開く"},
			{Cmd: "/unlink-account", Desc: "連携済み ID を表示"},
			{Cmd: "/user", Desc: "Web ユーザー管理"},
			{Cmd: "/version", Desc: "バージョン情報を表示"},
			{Cmd: "/prompt", Desc: "完全なプロンプトをプレビュー"},
			{Cmd: "/llms", Desc: "個人 LLM サブスクリプション一覧"},
			{Cmd: "/continue", Desc: "中断された前回ターンを続行"},
			{Cmd: "/context mode", Desc: "圧縮モードを表示または変更"},
			{Cmd: "/context", Desc: "コンテキスト統計を表示"},
			{Cmd: "/plugin reload-all", Desc: "全プラグインを再読み込み"},
		...
		HelpKeys: []HelpKeyEntry{
			...
			{Key: "Ctrl+N", Desc: "LLM パネル（モデル切替 + サブスクリプション管理）"},
