...
func TestConvertMessagesToHistoryUsesFinalAssistantHistoryIDAfterMerge(t *testing.T) {
	...
	// The merged row must ALSO carry the final assistant's stable DB ID. Without
	// this, HistoryMessage.ID is stale (10) or 0, so the JSON omits `id`
	// (json:"id,omitempty") and the frontend falls back to a batch-index temp ID
	// (hist-${i}) — loadMore dedup by id fails, duplicating this turn's iterations.
	if history[0].ID != 11 {
		t.Fatalf("merged assistant ID = %d, want 11 (must equal HistoryID so the frontend gets a stable dedup key)", history[0].ID)
	}
}

// TestConvertMessagesToHistory_SetsIDOnFlushPending verifies the tool_summary
// generated by flushPending() (intermediate ToolCalls assistants before a user
// message) carries a non-zero stable ID. Without ID, the frontend gets
// `hist-${batchIndex}` temp IDs that collide across loadMore batches → the same
// interrupted turn's iterations render twice after scrolling up.
func TestConvertMessagesToHistory_SetsIDOnFlushPending(t *testing.T) {
	history := ConvertMessagesToHistory([]llm.ChatMessage{
		{ID: 5, Role: "assistant", ToolCalls: []llm.ToolCall{{ID: "tc1", Name: "Shell", Arguments: "{}"}}},
		{ID: 6, Role: "user", Content: "hi"},
	})
	// The first row is the tool_summary from flushPending (ToolCalls assistant
	// accumulated then flushed by the user message). It must carry the stable ID.
	if len(history) < 1 {
		t.Fatalf("history = %+v, want at least 1 row", history)
	}
	summary := history[0]
	if summary.ID != 5 {
		t.Fatalf("tool_summary ID = %d, want 5 (lastAssistantID) — ID must be set so the frontend gets a stable dedup key", summary.ID)
	}
	if summary.HistoryID != 5 {
		t.Fatalf("tool_summary HistoryID = %d, want 5", summary.HistoryID)
	}
}
