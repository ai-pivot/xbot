...
// TestSyncProgressTodos_SameCountPreservesCache verifies the P3 fix:
// when todos have the same length but content changes (e.g. an item marked done),
// syncProgressTodos does NOT invalidate rc.valid. Previously it set rc.valid=false
// which triggered fullRebuild → O(all_messages) glamour on every todo status change.
