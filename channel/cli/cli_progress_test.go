...
func TestSyncProgressTodos_SameCountPreservesCache(t *testing.T) {
	...
	payload := &protocol.ProgressEvent{
		Todos: []protocol.TodoItem{
			{Text: "task-a", Status: "pending"},
			{Text: "task-b", Status: "done"}, // changed
			{Text: "task-c", Status: "pending"},
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			model.handleHistoryReload(cliHistoryReloadMsg{
				...
				forceFullRebuild: true,
				history: []channel.HistoryMessage{
					{ID: 11, RecordType: "message", Role: "assistant", Content: "original reply", Timestamp: now},
					tt.boundary,
				},
			})
			...
			if model.messages[len(model.messages)-2].historyID != tt.boundary.ID {
