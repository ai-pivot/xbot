...
func TestAppendOnlyHistoryRowsStayVisible(t *testing.T) {
	compacted := toCLIMessage(protocol.HistoryMessage{
		ID: 11, RecordType: "message", CompactedBy: 20, Role: "user", Content: "old",
	})
	marker := toCLIMessage(protocol.HistoryMessage{
		ID: 20, RecordType: "compress", Role: "system", Content: "[Compacted context]\nsummary",
	})
	internal := toCLIMessage(protocol.HistoryMessage{
		ID: 21, RecordType: "mask", Role: "control",
	})
	displayOnly := toCLIMessage(protocol.HistoryMessage{
		ID: 22, RecordType: "message", Role: "user", Content: "original", DisplayOnly: true,
	})
...
func TestHistoryProjectionRendersEmptyAssistantToolCall(t *testing.T) {
	...
	empty := toCLIMessage(protocol.HistoryMessage{ID: 32, RecordType: "message", Role: "assistant"})
	if !empty.hidden {
	...
	model.channel = &CLIChannel{
		config: &CLIChannelConfig{
			DynamicHistoryLoader: func(string, string) ([]channel.HistoryMessage, error) {
				return []channel.HistoryMessage{{ID: 11, RecordType: "message", Role: "user", Content: "rewrite"}}, nil
			},
		},
		asyncCh: make(chan tea.Msg, 1),
		stopCh:  make(chan struct{}),
...
func TestHistoryIDRewindBeforeCompressionKeepsVisibleSource(t *testing.T) {
	...
	model.channel = &CLIChannel{
		config: &CLIChannelConfig{
			...
			DynamicHistoryLoader: func(string, string) ([]channel.HistoryMessage, error) {
				return []channel.HistoryMessage{{ID: 10, RecordType: "message", CompactedBy: 30, Role: "user", Content: "earlier", Timestamp: time.Unix(1, 0)}}, nil
			},
		},
		asyncCh: make(chan tea.Msg, 1),
		stopCh:  make(chan struct{}),
