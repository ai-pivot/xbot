...
// truncateANSIVisible truncates ANSI-styled text to at most maxVisible visible
// runes. ANSI escape sequences (CSI: \x1b[...m and OSC: \x1b]...\x07) are
// skipped when counting — only visible characters count toward the limit.
//
// If truncation occurs, \x1b[0m (full reset) is appended to prevent color bleed
// from any open SGR sequence. This is the "insert a reset at the end" approach:
// it doesn't try to reconstruct the exact ANSI state at the truncation point,
// which would require a full ANSI state machine. The reset is sufficient because
// the next render either shows more content (twVisible increased) or the full
// content (stream finished), both of which start with their own styling.
//
// Used by the typewriter effect: glamour renders the FULL content once (cached),
// then each tick truncates the cached output to twVisible visible runes.
// This avoids re-running glamour.Render() on every 50ms tick when the content
// hasn't changed — only the typewriter position advanced.
func truncateANSIVisible(s string, maxVisible int) string {
	if maxVisible <= 0 {
		return ""
	}
	if s == "" {
	...
	visible := 0
	inCSI := false // \x1b[...m (SGR and other CSI sequences)
	inOSC := false // \x1b]...\x07 (OSC sequences, e.g. window title)
	truncIdx := -1
	for i, r := range s {
		if r == '\x1b' {
			inCSI = true
		...
		if inCSI {
			// CSI sequence: \x1b[...m  (or other final byte)
			// OSC sequence: \x1b]...\x07 (BEL) or \x1b]...\x1b\\ (ST)
			if r == ']' && !inOSC {
				inOSC = true
				continue
			}
			if inOSC {
				if r == '\x07' || (r == '\\' && i > 0 && s[i-1] == '\x1b') {
					inOSC = false
					inCSI = false
				}
				continue
		...
		// Visible character (including newlines — they affect layout)
		visible++
	...
	if truncIdx > 0 && truncIdx < len(s) {
		return s[:truncIdx] + "\x1b[0m"
	}
	return s // visible <= maxVisible, no truncation needed
}

// countANSIVisibleRunes counts visible (non-ANSI) runes in styled text.
// Used to compute the proportional truncation target for the typewriter effect.
func countANSIVisibleRunes(s string) int {
	visible := 0
	inCSI := false
	inOSC := false
	for i, r := range s {
