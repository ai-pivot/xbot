...
type SettingsCallbacks struct {
	...
	// ch.Subscription management (multi-subscription support)
	LLMListSubscriptions      func(senderID string) ([]ch.Subscription, error)  // list all user subscriptions
	LLMGetDefaultSubscription func(senderID string) (*ch.Subscription, error)   // get active subscription
	LLMAddSubscription        func(senderID string, sub *ch.Subscription) error // add a new subscription
	LLMUpdateSubscription     func(id string, sub *ch.Subscription) error       // update an existing subscription
...
func (f *FeishuChannel) Send(msg ch.OutboundMsg) (string, error) {
	if f.client == nil {
...
func (f *FeishuChannel) onCardAction(ctx context.Context, event *callback.CardActionTriggerEvent) (*callback.CardActionTriggerResponse, error) {
	...
	// Intercept AskUser card actions.
	if resp, ok := f.handleAskUserCardAction(actionData, action, chatID, senderID, messageID); ok {
		return resp, nil
	}
...
// ---------------------------------------------------------------------------
// AskUser: interactive card for asking user questions
// ---------------------------------------------------------------------------

const askUserActionPrefix = "askuser_"
...
func (f *FeishuChannel) sendAskUserCard(msg ch.OutboundMsg) (string, error) {
	var questions []ch.AskQItem
	if msg.Metadata != nil {
		if qJSON := msg.Metadata["ask_questions"]; qJSON != "" {
			json.Unmarshal([]byte(qJSON), &questions)
		}
	}
	if len(questions) == 0 {
		return "", fmt.Errorf("ask_user card: no questions in metadata")
	}

	card := f.buildAskUserCard(questions)
	cardJSON, err := json.Marshal(card)
	if err != nil {
...
func (f *FeishuChannel) buildAskUserCard(questions []ch.AskQItem) map[string]any {
	elements := []map[string]any{}
...
func (f *FeishuChannel) handleAskUserCardAction(actionData map[string]any, action *callback.CallBackAction, chatID, senderID, messageID string) (*callback.CardActionTriggerResponse, bool) {
	...
	actionVal, _ := actionData["ask_user_action"].(string)
	if actionVal == "" {
		return nil, false
	}
...
func (f *FeishuChannel) handleAdminCommand(content, replyTo, senderID, messageID string) bool {
	...
	case "!webadd":
		...
		createdName, password, err := web.CreateWebUser(f.webDB, username)
		if err != nil {
...
// --- ch.SettingsCapability implementation ---

// feishuSettingsSchema returns the settings definitions for Feishu channel.
func feishuSettingsSchema() []ch.SettingDefinition {
	loc := ch.GetLocale(ch.CurrentLocaleLang())
	base := append([]ch.SettingDefinition(nil), loc.SettingsSchema...)
	base = append(base, ch.SettingDefinition{
		Key:         "sandbox_cleanup",
		Label:       "沙箱持久化",
		Description: "将当前沙箱文件系统变更保存为镜像（export+import）。执行期间会阻塞你的所有请求。",
		Type:        ch.SettingTypeToggle,
		Category:    "沙箱",
	})
	return base
}
...
func (f *FeishuChannel) SettingsSchema() []ch.SettingDefinition {
...
// --- ch.UIBuilder implementation ---

// BuildSettingsUI builds a Feishu card for settings display.
func (f *FeishuChannel) BuildSettingsUI(ctx context.Context, schema []ch.SettingDefinition, currentValues map[string]string) string {
	var sb strings.Builder
	sb.WriteString("## ⚙️ 设置\n\n")

	// Group by category
	categories := make(map[string][]ch.SettingDefinition)
	...
	for cat, defs := range categories {
		...
		for _, def := range defs {
			...
			if def.Type == ch.SettingTypeSelect && len(def.Options) > 0 {
				sb.WriteString("可选：")
...
func (f *FeishuChannel) BuildMainMenuUI(ctx context.Context, senderID string) string {
	...
	sb.WriteString("- 📦 `/app` — 应用管理（打包、安装、卸载）\n")
	sb.WriteString("- 📥 `/app install <file|url>` — 安装应用\n")
